...
//                 <Link
//                     href="/shop"
//                     className="block w-full text-center px-6 py-3 border border-neutral-400 hover:bg-neutral-50 text-black font-medium rounded-lg transition-colors"
//                 >
//                     Continue Shopping
//                 </Link>
...
export default function CartPage() {
    ...
    return (
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 mt-20">
            ...
            <div className="lg:col-span-2 space-y-4">
                {items.map((item) => (
                <div key={`${item.product.id}-${item.selectedColor ?? ''}-${item.selectedSize ?? ''}`} className="bg-white rounded-lg p-6 shadow-sm">
                    <div className="flex gap-6">
                    {/* Product Image */}
                    <div className="relative w-24 h-24 sm:w-32 sm:h-32 flex-shrink-0 rounded-lg overflow-hidden bg-neutral-200">
                        <Image
