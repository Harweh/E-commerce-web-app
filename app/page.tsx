...
import { ShoppingBag, Mail, Star, Ticket, Heart, Store, Eye, Search, CreditCard, 
    LogOut, MessageSquare, Phone, ChevronRight} from 'lucide-react'
import { useAuthStore } from '@/store/auth'

// Zustand's persist middleware hydrates from localStorage asynchronously,
// so on first render isAuthenticated is always false even for a logged-in
// user. useSyncExternalStore lets us subscribe to the hydration event
// directly instead of tracking it with a setState-in-effect pattern.
function useAuthHasHydrated() {
...
export default function AccountPage() {
    ...
    // Not logged in — the effect above is already redirecting to /login
    if (!isAuthenticated || !user) {
        return null
    }

    return (
        <div className="min-h-screen bg-gray-50 pt-36 mt-12 sm:pt-27 pb-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <button className="bg-gray-500 hover:bg-orange-600 text-white rounded-lg py-3 px-4 flex items-center justify-center gap-2 font-semibold transition-colors">
